...
%
%   Skips if parcluster is missing, then unless AdditionalProperties has
%   ComputingProject (CSC generic profile). Then asserts
%   configure_cluster_profile sets the project name (and related fields).
%

    methods (Test)
        function testConfigureClusterProfileSetsCscFields(testCase)
            testCase.assumeTrue(exist('parcluster', 'file') == 2, ...
                'Skipping: Parallel Computing Toolbox parcluster is not available.');
            c = parcluster;
