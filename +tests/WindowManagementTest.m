...
%
    ...
    methods (Test)
        ...
        function menuMinimizeDoesNotWalkOffScreen(testCase)
            menuFig = testCase.track(uifigure('Visible', 'off', 'Name', 'ZEFFIRO Interface: Menu tool'));
            menuFig.Position = [100 400 500 0];
            if ~isprop(menuFig, 'ZefMenuMinHeight')
                addprop(menuFig, 'ZefMenuMinHeight');
            end
            menuFig.ZefMenuMinHeight = menuFig.Position(4);
            zef = struct();
            zef.h_zeffiro_menu = menuFig;
            zef.h_menu_logo = uilabel(menuFig);
            zef.menu_expanded_size = 220;
            y0 = menuFig.Position(2);
            for k = 1:8
                zef_set_menu_size(zef, 'minimized');
            end
            testCase.verifyEqual(menuFig.Position(2), y0, 'AbsTol', 2);
            testCase.verifyLessThanOrEqual(menuFig.Position(4), menuFig.ZefMenuMinHeight + 1);

            zef_set_menu_size(zef, 'expanded');
            testCase.verifyGreaterThan(menuFig.Position(4), menuFig.ZefMenuMinHeight + 1);
            zef_set_menu_size(zef, 'minimized');
            testCase.verifyLessThanOrEqual(menuFig.Position(4), menuFig.ZefMenuMinHeight + 1);
            testCase.verifyEqual(menuFig.Position(2), y0, 'AbsTol', 2);
        end
