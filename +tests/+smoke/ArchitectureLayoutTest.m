...
%   not; that L1_optimization / LG_optimization live under src/inverse; that
%   retired folders tools/plugins, src/core, src/gui/helpers,
%   src/auxiliary, +plugins, and plugins/GithubPusher are absent, as are
%   local dump folders, root screenshot helpers, Untitled.mat, the retired
%   PNG icon rasterizer, and dark-theme moon/sun PNG icons.
    ...
    methods (Test)
        function publicEntryPointsResolveToOwnedFolders(testCase)
            ...
            cases = { ...
                ...
                'zef_close_waitbar', fullfile('src', 'app'); ...
                'zef_ensure_parpool', fullfile('src', 'app'); ...
                'zef_read_profile_cell', fullfile('src', 'app'); ...
                'zef_window_manager', fullfile('src', 'gui', 'chrome'); ...
                'zef_ui_theme', fullfile('src', 'gui', 'chrome'); ...
                'zef_ui_icons', fullfile('src', 'gui', 'chrome'); ...
                'zef_figure_interact', fullfile('src', 'gui', 'chrome'); ...
                'zef_ui_blend_logo', fullfile('src', 'gui', 'chrome'); ...
                ...
                'zef_lattice_cubes_to_tetra', fullfile('src', 'mesh'); ...
                'zef_find_gaussian_prior', fullfile('src', 'inverse'); ...
                'L1_optimization', fullfile('src', 'inverse'); ...
                'LG_optimization', fullfile('src', 'inverse'); ...
        ...
        function retiredDirectoriesAreGone(testCase)
            ...
            testCase.verifyTrue(isfile(fullfile(root, 'scripts', 'run_tests_batch.m')));
            testCase.verifyTrue(isfile(fullfile(root, 'scripts', 'validate_stl_manifold.py')));
            testCase.verifyFalse(isfile(fullfile(root, 'scripts', 'zef_gui_audit.m')));
            testCase.verifyFalse(isfile(fullfile(root, 'scripts', 'refresh_zef_ui_icons.py')));
            ...
            testCase.verifyFalse(isfolder(fullfile(root, 'assets', 'fig', 'ui', 'Zeffiro_Modern_Icons')));
            testCase.verifyFalse(isfile(fullfile(root, 'assets', 'fig', 'ui', 'moon.png')));
            testCase.verifyFalse(isfile(fullfile(root, 'assets', 'fig', 'ui', 'sun.png')));
        end
