...
if length(n_map_iterations) < multires_n_levels
    ...
    % multi-solver sensitivity work: ensure the iteration variable `f` is
    % defined on the CPU path too. Previously `f` was only assigned inside
    % the GPU branch, so the L1_optimization call below referenced an
    % undefined variable for any non-GPU dispatch.
    f = f_data;
    if opts.use_gpu == 1 && gpuDeviceCount > 0
        f = gpuArray(f_data);
    end
        ...
        if sum(isnan(z_vec))>0
            if ~nan_map_warned
                warning('HALpRInverter:NaNInMapIteration', ...
                    'NaN in z_vec during HALpR MAP iterations (first at iteration %d of %d). Replacing NaNs with finite means.', i, n_map_iterations(j));
                nan_map_warned = true;
            end
