...
%
% zef_rec_diff - Compute position, direction, magnitude, and dispersion differences.
%
% For each source position, generates synthetic measurements along three
% orthogonal directions, runs the given inverse method, and computes:
%   - dist_vec:      Position error (L2 or minabs metric)
%   - angle_vec:     Direction error (degrees)
%   - mag_vec:       Reconstructed magnitude
%   - dispersion_vec: Spatial spread of reconstructed activity within ROI
%
% Inputs:
%   zef             Project struct with lead field and source positions
%   inverse_method  Function handle (e.g. @zef_find_mne_reconstruction)
%   noise_db        Noise level in dB (non-positive, 0 = no noise)
%   diff_type       "L2" or "minabs" for position error metric
%   dispersion_radius  Radius (mm) for ROI dispersion computation
%
% Outputs:
%   dist_vec, angle_vec, mag_vec, dispersion_vec  Column vectors
%
