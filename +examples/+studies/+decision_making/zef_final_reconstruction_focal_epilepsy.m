% ZEF_FINAL_RECONSTRUCTION_FOCAL_EPILEPSY - Combine selected reconstructions into final result
%
% Duplicates inverse results (max point + cluster centre types), then keeps
% only those selected by zef_cluster_reconstructions_focal_epilepsy (J_aux).
% Computes final reconstruction as sum of selected reconstructions and
% extracts max point and cluster statistics for visualization.
%
% Requires: z_inverse_results, z_inverse_info, J_aux, GMModel, max_ind from
% zef_cluster_reconstructions_focal_epilepsy.
%
% Sets: z_final, z_final_max_point, z_cluster_mean, z_cluster_deviation,
% z_ref_points, z_max_deviations, z_mean_deviations for zef_show_results.
%
% See also: zef_cluster_reconstructions_focal_epilepsy, zef_show_results_focal_epilepsy
