...
%
% zef_ES_centralize_recursive_search - Narrow alpha/epsilon range around best point.
%
% Given current alpha and epsilon grids and the best indices (sr, sc) from
% the objective function, computes a reduced parameter range centered on
% that point. Used by zef_ES_recursive_search for adaptive tES hyperparameter
% optimization.
%
% Inputs:
%   alpha, epsilon    Current parameter grids
%   sr, sc            Row/column indices of best objective value
%   original_window   Window size for parameter search
%   s_alpha, s_epsilon  Shrinkage factors
%   varargin{1}       Optional: non_floating_flag (1 = clamp to [min,max])
%
% Outputs:
%   alpha_psi, epsilon_psi  New parameter grids for next iteration
%
% See also: examples.studies.tES_hyperparameter_optimization.zef_ES_recursive_search
%
