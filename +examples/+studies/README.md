...
| `+tES_hyperparameter_optimization/` | `zef_ES_recursive_search(zef, num_lattice)` | Shrink ES-workbench α/ε lattice — **distinct** from plugin `plugins/ZeffiroESWorkbench/m/zef_ES_recursive_search` (GUI 3-arg) |
...
Depends on `plugins` (Data Bank, GMM, ES Workbench, …), `src/forward/lead_field`, and sometimes cluster utilities. Not started from default menus.
...
- Not part of the `tests` package suite.
