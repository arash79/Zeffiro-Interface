...
MUSIC **does** call `zef_blocked_source_index` for location→column maps after its own L subset. For Mesh-tool **Directions = Normal** it still rebuilds `s_ind_4` itself by looping hardcoded compartments `d1`…`d22`, `w`, `g`, `c`, `sk`, `sc` (`k = 1:27`). Extra `zef.compartment_tags` never enter that normal constraint. Band-pass is an inline elliptic filter, not `zef_getFilteredData`.
