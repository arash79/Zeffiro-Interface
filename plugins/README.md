...
Class-based solvers in `+inverse` are a **different** track: Inverse-tools items without **(class solver)** still call the legacy iterations listed here. Conceptual comparison: [docs/methods.md](../docs/methods.md).

## Main contents

Typical plugin folder: start callback (`*_start.m`), `m/` (iteration / math), `mlapp/` or `fig/` (window layout), optional README.

**Default Inverse tools** (`profile/multicompartment_head/zeffiro_plugins.ini`): Minimum norm (`zef_minimum_norm_estimation` / `zef_find_mne_reconstruction`), Classical Sparse Methods, IAS / IAS ROI, RAMUS, Dipole Scan, Beamformer, Kalman, MUSIC, Hierarchical Bayesian Sampler, Preconditioned relaxation, SL1 MAP, EXP Lasso, GMM (SP/JL), ES Workbench (tES optimization), SESAME, RAP-MUSIC, plus **(class solver)** rows for eLORETA, UKF-NMM, HALpR, Group Lasso, MNE, IAS, RAMUS, CSM, Kalman, Beamformer, and Dipole Scan. Class-solver Start functions open `zef_open_class_inverse` and run `zef_inverse_run`; they are not legacy `*_iteration` wrappers.
...
Almost every **legacy** inverse plugin: `zef_processLeadfields` (needs `zef.L`), `zef_getFilteredData` / `zef_getTimeStep` (needs `zef.measurements`), writes `zef.reconstruction` and `zef.reconstruction_information`. **(class solver)** folders skip that loop and call `zef_inverse_run` from `zef_open_class_inverse`.
...
- GUI Kalman is `zef_KF`; class Kalman is `inverse.KalmanInverter` (registry `kalman` / `kf`)—the Kalman menu without **(class solver)** does not construct the class path.
- **(class solver)** menus *do* call `zef_inverse_run`. Pattern folders: `plugins/ELORETA`, `plugins/UKFNMM`, `plugins/HALpR`, `plugins/GroupLasso`, plus `zef_*_class_start` wrappers in MNE, IAS, RAMUS, CSM, Kalman, Beamformer, and Dipole Scan.
