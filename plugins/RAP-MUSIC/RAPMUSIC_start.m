...
%
%   Script. Call from MATLAB. Cluster registry id legacy_rap_music
%   dispatches RAP_MUSIC_iteration, not this start script.
...
zef.RAPMUSIC.inv_sampling_frequency.Value = zef_ui_num2str(zef.inv_sampling_frequency);
zef.RAPMUSIC.inv_low_cut_frequency.Value =  zef_ui_num2str(zef.inv_low_cut_frequency);
zef.RAPMUSIC.inv_high_cut_frequency.Value =  zef_ui_num2str(zef.inv_high_cut_frequency);
zef.RAPMUSIC.inv_time_1.Value =  zef_ui_num2str(zef.inv_time_1);
zef.RAPMUSIC.inv_time_2.Value = zef_ui_num2str(zef.inv_time_2);
zef.RAPMUSIC.number_of_frames.Value = zef_ui_num2str(zef.number_of_frames);
zef.RAPMUSIC.inv_time_3.Value = zef_ui_num2str(zef.inv_time_3);
...
for zef_i = 1:length(zef_props)
    if isfield(zef,zef_props{zef_i})
        try
            zef.RAPMUSIC.(zef_props{zef_i}).Value = zef_ui_num2str(zef.(zef_props{zef_i}));
        catch
        end
    end
end
...
zef.RAPMUSIC.StartButton.ButtonPushedFcn = '[zef.reconstruction,~,zef.reconstruction_information] = RAP_MUSIC_iteration;';
...
zef_ui_adopt_app(zef.RAPMUSIC.UIFigure, 'ZEFFIRO Interface: RAP-MUSIC');
