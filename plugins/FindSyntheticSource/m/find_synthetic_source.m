...
try
    fig = zef.find_synth_source.h_find_synth_source;
    fig.Name = 'ZEFFIRO Interface: Find synthetic source';
    fig.SizeChangedFcn = '';
    try
        zef_props = properties(zef.find_synth_source);
        for zef_i = 1:numel(zef_props)
            w = zef.find_synth_source.(zef_props{zef_i});
            if isgraphics(w) && isvalid(w) && isprop(w, 'Tag')
                w.Tag = zef_props{zef_i};
            end
        end
    catch
    end
