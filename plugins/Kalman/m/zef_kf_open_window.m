...
for zef_i = 1:length(zef_props)
    if isfield(zef,zef_props{zef_i})
        try
            zef_val = zef.(zef_props{zef_i});
            if isnumeric(zef_val) || islogical(zef_val)
                zef.KF.(zef_props{zef_i}).Value = num2str(zef_val);
            elseif ischar(zef_val) || isstring(zef_val)
                zef.KF.(zef_props{zef_i}).Value = char(string(zef_val));
            end
        catch
        end
...
try
    zef.KF.number_of_ensembles.Visible = 'off';
    zef.KF.NumberofensemblesLabel_2.Visible = 'off';
catch
end
...
try
    zef.KF.UIFigure.Name = 'ZEFFIRO Interface: Kalman';
catch
end
