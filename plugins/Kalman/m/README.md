...
Legacy **Inverse tools → Kalman** implementation: discrete-time Kalman / EnKF / block-sLORETA filters on the source vector, with optional **DTI structural process noise Q**. Parallel to — and **not** the same as — `inverse.KalmanInverter` / `inverse.kf`.
...
| Smoothers | `RTS_smoother`, `Block_RTS_smoother` |
| Q / DTI | `find_evolution_prior` (via `zef_KF`), `zef_dti_structural_Q`, `zef_dti_interpolate_to_sources`, `zef_dti_fa_covariance`, `zef_dti_tractography_covariance` |
...
- Pitfall: comparing plugin KF to `inverse.KalmanInverter` without matching `R`, `Q`, and smoother settings. Class Kalman has **no** DTI structural Q.
