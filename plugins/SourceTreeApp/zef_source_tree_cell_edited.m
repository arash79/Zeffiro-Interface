...
%ZEF_SOURCE_TREE_CELL_EDITED  Copy a parameter-table edit onto NodeData or global signal params.
%
%   zef_source_tree_cell_edited(~, tbl, event, whichTable)
%
%   whichTable is "SourceParameters" (selected node's NodeData) or
%   "SignalParameters" (zef.source_tree_signal_parameters). Reads zef from
%   base, writes the edited Value onto the matching Name/Value leaf, then
%   rebuilds zef.source_tree from the UI tree and assignin('base','zef',zef).
%
%   Numeric / logical table strings are cast back to the stored type.
%
%   See also zef_source_tree_selection_changed, zef_build_source_tree.
