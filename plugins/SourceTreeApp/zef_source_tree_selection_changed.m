...
%ZEF_SOURCE_TREE_SELECTION_CHANGED  Fill the SourceParameters table from NodeData.
%
%   App Designer SelectionChangedFcn for the source tree. Reads zef from
%   the base workspace. Writes {KeyPath, Value} rows onto
%   zef.h_source_tree.SourceParameters.Data from the selected node's
%   Name/Value leaves, and stores dropdown suggestions in that table's
%   UserData. Empty selection or empty NodeData clears the table.
%
%   Does not assignin zef (only the table Data / UserData change).
%
%   See also zef_source_tree_cell_edited, zef_init_jr_nodedata.
