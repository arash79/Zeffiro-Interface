...
%   Needs zef.reconstruction and zef.source_positions. Weighted EM is
%   inverse.gmm.FitAdvGMM. Waitbar is MATLAB waitbar (not
%   zef_waitbar). Does not invert L.
%
%   See also zef_GMModeling_K, zef_GMM_AdvModelingOpt.

h = waitbar(0,['Gaussian mixture model.']);
...
for t=t_start:T
    ...
    if strcmp(initial_mode,'1')
        % k-means++ start (Start='plus') through inverse.gmm.FitAdvGMM.
        % Retry with RegularizationValue if the unregularized fit throws.
        try
            GMModel_aux = inverse.gmm.FitAdvGMM(activity_space,weight,k,'CovarianceType',Sigma, ...
