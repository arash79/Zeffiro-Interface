...
%   Not an inverse solver and not inverse.gmm.
%
...
for zef_i = 2:length(zef_props)
    if strcmp(zef.GMM.apps.main.(zef_props{zef_i-1}).Type,'uilabel')
        ...
        if isfield(zef,zef_props{zef_i})
            try
            zef.GMM.apps.main.(zef_props{zef_i}).Value = zef_ui_num2str(zef.(zef_props{zef_i}));
            if isempty(zef.GMM.apps.main.(zef_props{zef_i}).Value) ...
                    && (ischar(zef.(zef_props{zef_i})) || isstring(zef.(zef_props{zef_i})))
                zef.GMM.apps.main.(zef_props{zef_i}).Value = char(zef.(zef_props{zef_i}));
            end
            zef_GMM_label_names{zef_n}=zef.GMM.apps.main.(zef_props{zef_i-1}).Text;
...
zef.GMM.apps.main.ImportMenu.MenuSelectedFcn = 'if not(isempty(zef.save_file_path)) && prod(not(zef.save_file_path==0)); [zef_aux_file,zef_aux_path] = uigetfile(''*.mat'',''Select Gaussian Mixture Model'',zef.save_file_path); else; [zef_aux_file,zef_aux_path] = uigetfile(''*.mat'',''Select Gaussian Mixture Model''); end; if ~isequal(zef_aux_file,0) && ~isequal(zef_aux_path,0); zef_aux = load(fullfile(zef_aux_path,zef_aux_file)); if isfield(zef_aux,''zef_GMModel''); zef.GMM.model = zef_aux.zef_GMModel; zef.GMM.dipoles = zef_aux.zef_GMModelDipoles; elseif isfield(zef_aux,''zef_GMM''); zef_load_GMM(zef_aux.zef_GMM); zef_GMM_update; end; end; clear zef_aux zef_aux_file zef_aux_path;';
...
zef_ui_adopt_app(zef.GMM.apps.main.UIFigure, 'ZEFFIRO Interface: Gaussian Mixture Model (JL)');
