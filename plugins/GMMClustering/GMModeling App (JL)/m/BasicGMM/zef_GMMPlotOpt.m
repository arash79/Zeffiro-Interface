...
for zef_i = 2:length(zef_props)
    if strcmp(zef.GMM.apps.PlotOpt.(zef_props{zef_i-1}).Type,'uilabel')
        ...
        if ~strcmp(zef_props{zef_i},'GMM_colors') && ~isempty(zef.GMM.parameters{zef_n,2}{1})
            ...
            elseif ~isempty(zef.GMM.parameters{zef_n,2}{1})
                zef.GMM.apps.PlotOpt.(zef_props{zef_i}).Value=zef.GMM.parameters{zef_n,2}{1};
            end
        end 
...
zef_ui_adopt_app(zef.GMM.apps.PlotOpt.UIFigure, 'ZEFFIRO Interface: Advanced GMM plot options');
