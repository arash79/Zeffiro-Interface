...
for f_ind = 1 : number_of_frames
    ...
    for i = 1 : n_ias_map_iter
        ...
        % Weighted MNE operator: √θ L' (L θ L' + σ² I)⁻¹. Skip forming the
        % n_dof×n_sensors filter when only z = W*f is required.
        Wd = L_aux .* d_sqrt';
        A = Wd * Wd' + S_mat;
        need_full_W = isequal(ias_type, 2) || isequal(ias_type, 4) ...
            || (i == n_ias_map_iter && (isequal(ias_type, 3) || isequal(ias_type, 5)));
        if need_full_W
            L = d_sqrt.*( Wd' * inv( A ) );
