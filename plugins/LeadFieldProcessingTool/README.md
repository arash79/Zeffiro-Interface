...
Start script `LeadFieldProcessingTool_start.m` (plugin root) plus bank helpers under `m/` — **Add**, **Replace**, mag→grad via `tra`, noise-weighted **Combine**, delete/refresh/table. Mixed identifier spelling `LeadField` vs `Leadfield` is load-bearing. Full file map: [`m/README.md`](m/README.md).
