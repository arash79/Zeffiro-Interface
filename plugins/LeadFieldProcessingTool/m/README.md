...
| `zef_LeadfieldProcessingTool_loadTra.m` | Load magnetometer→gradiometer `tra` |
...
| `zef_LeadfieldProcessingTool_delete.m` | **Delete** checked bank rows |
| `zef_LeadfieldProcessingTool_refresh.m` | **Refresh** table from live `zef` |
| `zef_LeadfieldProcessingTool_updateTable.m` | Rebuild bank `uitable` contents |
| `zef_LeadfieldProcessingTool_BankTableLabelUpdate.m` | Refresh bank-table labels |
| `zef_LeadFieldProcessingTool_aux2bank_new.m` | Create a new bank entry from aux |
| `zef_LeadfieldProcessingTool_bank2aux_bank2auxIndex.m` | Copy a bank row into aux |
...
- Wire new buttons only through the start/window script.
