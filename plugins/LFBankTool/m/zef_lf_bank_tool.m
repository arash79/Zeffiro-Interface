...
zef.h_merge_lead_fields.ButtonPushedFcn = '[zef.yesno] = zef_ui_confirm(''Merge selected lead fields?'',''Yes'',''No''); if isequal(zef.yesno,''Yes''); zef_combine_lead_fields; end;';
zef.h_delete_selected.ButtonPushedFcn = '[zef.yesno] = zef_ui_confirm(''Delete selected lead field items?'',''Yes'',''No''); if isequal(zef.yesno,''Yes''); zef_delete_lf_item; end;';
...
zef.h_lf_bank_compute_lead_fields.ButtonPushedFcn = '[zef.yesno] = zef_ui_confirm(''Re-calculate the lead field matrices for the selected items?'',''Yes'',''No''); if isequal(zef.yesno,''Yes''); zef_lf_bank_compute_lead_fields; end;';
zef.h_lf_bank_update_measurements.ButtonPushedFcn = '[zef.yesno] = zef_ui_confirm(''Substitute the measurement data of the selected lead field items with the current measurement data?'',''Yes'',''No''); if isequal(zef.yesno,''Yes''); zef_lf_bank_update_measurements; end;';
zef.h_lf_bank_update_noise_data.ButtonPushedFcn = '[zef.yesno] = zef_ui_confirm(''Substitute the noise data of the selected lead field items with the current noise data?'',''Yes'',''No''); if isequal(zef.yesno,''Yes''); zef_lf_bank_update_noise_data; end;';
zef.h_lf_bank_make_all.ButtonPushedFcn = '[zef.yesno] = zef_ui_confirm(''Create a mesh and calculate the lead field matrices for the selected items?'',''Yes'',''No''); if isequal(zef.yesno,''Yes''); zef.source_interpolation_on = 1; if isfield(zef,''h_source_interpolation_on''); if isvalid(''zef.h_source_interpolation_on''); set(zef.h_source_interpolation_on,''value'',1); end; end; zef_process_meshes; zef_create_fem_mesh; zef_postprocess_fem_mesh; zef.n_sources_mod = 1; zef.source_ind = []; zef_update_fig_details;zef_lf_bank_compute_lead_fields; end;';
zef_init_lf_bank_tool;

try
    zef.h_lf_bank_tool.Name = 'ZEFFIRO Interface: Multi lead field tool';
    zef.h_lf_bank_tool.SizeChangedFcn = '';
    zef_ui_ready(zef.h_lf_bank_tool);
