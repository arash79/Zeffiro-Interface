...
Data-bank open helpers, tree refresh / get-set data, combine lead fields, import/export, and working-hash table support. Entry: `zef_start_dataBank` → `zef_open_dataBank`. App Designer: `zef_dataBank_app.mlapp`, `zef_dataBank_nameChange_app.mlapp`. Full `.m` map and entry-type table: [`m/README.md`](m/README.md).
...
MATLAB Combine and tree-selection helpers dispatch by filename (`zef_dataBank_combineLeadFields`, `zef_dataBank_getHashForMenu`).
...
Do not rename Combine or tree-selection filenames. Keep entry-type list and combine modes (`frobenius`, `fuchs`, `whitening`) stable.
