...
%ZEF_FIND_SOURCE_ROI  Synthetic measurements from dipoles inside one or more ROIs.
%
%   Zeffiro Interface.
...
%
%   Reads zef.synth_source_ROI (Find synthetic extended source widgets),
%   finds interpolated sources in each ROI via zef_ROI_finder, orients
%   them (manual / SVD max / SVD min / cortical normal / patch normal),
%   splits each ROI amplitude equally, and forms
%     meas = L(:, 3-column groups) * stacked_moments + noise.
%   Noise is noise_level * max(abs(meas)) * randn, i.e. a fraction of peak,
%   not an SNR in dB.
%
%   [meas, pos, ori, amp, n_per_roi, specs] = zef_find_source_ROI(zef)
%   [meas, pos, ori, amp, n_per_roi, specs] = zef_find_source_ROI(zef, false)
%
%   The optional second argument (default true) allows a source to belong
%   to more than one ROI. false keeps the first ROI's claim and errors if
%   a later ROI is emptied.
%
%   Cortical-normal modes call zef_processLeadfields and expect a
%   constrained grey-matter compartment (*_sources == 1).
%
%   See also zef_ROI_finder, zef_minmax_svd_ori, zef_find_synthetic_source_ROI.
