...
%ZEF_PLOT_DISK  Preview a flat cylinder or spherical-cap ROI on the current axes.
%
%   h_surf = zef_plot_disk(position, radius, width, ori, k, color)
%
%   position  - 1×3 centre (same length unit as source_positions).
%   radius    - disk radius.
%   width     - thickness along ori (flat case) or cap thickness (curved).
%   ori       - axis direction (normalized internally).
%   k         - curvature in [-1, 1]. k==0 draws a cylinder (ring + caps).
%               Nonzero k draws inner/outer spherical caps with sphere
%               radius radius/(k*sqrt(2)) (sign of k flips the cap),
%               matching zef_ROI_finder.
%   color     - FaceColor for the semi-transparent surfs.
%
%   Returns an hggroup. Does not write zef.
%
%   See also zef_plot_source_ROI, zef_ROI_finder.
