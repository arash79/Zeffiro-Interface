...
%ZEF_GET_RADIAL_ORIENTATION  SVD orientations with a consistent hemisphere.
%
%   'max': zef_minmax_svd_ori then flip any source whose lead-field pattern
%   peaks negative. 'min': requires s_ref and source_positions; uses the
%   vector from the cloud centroid through s_ref as radialRef for
%   zef_minmax_svd_ori.
%
%   ori = zef_get_radial_orientation(s_roi, 'max', L)
%   ori = zef_get_radial_orientation(s_roi, 'min', L, s_ref, source_positions)
%
%   See also zef_minmax_svd_ori, zef_find_source_ROI.
