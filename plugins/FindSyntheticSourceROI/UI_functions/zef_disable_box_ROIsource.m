...
%ZEF_DISABLE_BOX_ROISOURCE  Enable/disable ROI widgets from the shape dropdown.
%
%   zef = zef_disable_box_ROIsource(zef)
%
%   shapeValue 1 = flat disk (width + curvature on), 2 = sphere (those
%   off; patch-surface-normal dipole option is removed), 3 = ellipsoid
%   (width on, curvature off). Custom / cortex-normal orientation edits
%   stay on only when they apply. Plot-style 2 (classic dipoles) enables
%   the length box. Mutates widget Enable/String/Value only.
%
%   See also zef_check_curvature_range, zef_ROI_finder.
