...
%   zef.exp_multires_* from base, not EXP.parameters). ApplyButton
%   re-runs each widget ValueChangedFcn. Numeric exp_* ValueChangedFcn
%   uses str2double.
%
    ...
    if contains(zef.EXP.app.(props{n}).Type,'editfield')
        if isfield(zef,props{n})    %load values from zef
            if isnumeric(zef.EXP.app.(props{n}).Value)
                ...
                zef.EXP.app.(props{n}).Value = zef_ui_num2str(zef.(props{n}));
        ...
        elseif isfield(zef.EXP.parameters,props{n})     %if the parameters field exists and have the values
            ...
            zef.EXP.app.(props{n}).Value = zef_ui_num2str(zef.EXP.parameters.(props{n}));
            end
        ...
        if startsWith(props{n},'exp')
            if isnumeric(zef.EXP.app.(props{n}).Value)
                ...
                zef.EXP.app.(props{n}).ValueChangedFcn = ['zef.EXP.parameters.',props{n},'=str2double(zef.EXP.app.',props{n},'.Value);'];
            end
...
zef.EXP.app.ApplyButton.ButtonPushedFcn = ['props=properties(zef.EXP.app); for n=1:numel(props); try; fcn=zef.EXP.app.(props{n}).ValueChangedFcn; if ~isempty(fcn); eval(fcn); end; catch; end; end;'];
...
zef_ui_adopt_app(zef.EXP.app.UIFigure, 'ZEFFIRO Interface: Hierarchical L1/L2 MAP Inversion');
