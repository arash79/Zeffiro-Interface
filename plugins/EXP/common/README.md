...
|-------------------|--------------------------------------|------------|
| 1 | EXP IAS | Sequential hyperprior MAP (`L1_optimization` when `q==1`, else weighted L2) |
| 2 | EXP EM | Same inner loops as IAS; tag `EXP EM` |
...
Default head-profile EXP entry. Asteroid / `_legacy` profiles open the older GUIDE **EXP IAS RAMUS** window instead (`exp_ias_multires/`). Not `inverse.GroupLassoInverter`. Registry id `legacy_exp` still dispatches `exp_iteration` on the cluster path.
...
- Numeric `exp_*` ValueChangedFcn in `zef_exp_app_start` uses `str2double`.
- **Apply** re-runs each widget ValueChangedFcn.
