...
%EXP_ITERATION  Hierarchical L1/L2 MAP (IAS / EM / sLORETA) for EXP.
%
%   Zeffiro Interface.
...
%
%   [z, reconstruction_information] = exp_iteration(zef)
%
%   Called from EXP StartButton (legacy_exp; not inverse.GroupLassoInverter).
%   eval's zef.EXP.parameters after stripping the 'exp_' prefix (q,
%   n_map_iterations, n_L1_iterations, hypermode, use_multires, …).
%   Needs zef.L and measurements. Frames: zef.number_of_frames.
%   SNR: zef.inv_snr → S_mat = (10^(-inv_snr/20))^2 * max(f_data.^2) * I.
%   estimation_type 1 EXP IAS / 2 EXP EM / 3 EXP sLORETA; use_multires
%   appends ' Multiresolution'. q==1 → L1_optimization; else weighted
%   L2. Returns z (cell per frame) and reconstruction_information.
%
%   See also zef_exp_app_start, L1_optimization, LG_optimization.
