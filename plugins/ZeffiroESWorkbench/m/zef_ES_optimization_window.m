...
zef.h_ES_find_currents_button.ButtonPushedFcn    = 'question_aux = zef_ui_confirm(''Confirm calculations?'',''ZI''); switch question_aux; case ''Yes''; if     zef.h_ES_HPO_search_method.Value == 1; zef = zef_ES_find_currents(zef); elseif zef.h_ES_HPO_search_method.Value == 2; zef = zef_ES_find_currents_recursive(zef); end;; end;;';
%% Right-click
...
try
    zef.h_ES_workbench.SizeChangedFcn = '';
    zef_ui_apply_size(zef.h_ES_workbench, 580, 820, 500, 700);
    zef.h_ES_workbench_current_size = zef_change_size_function(zef.h_ES_workbench, ...
        zef.h_ES_workbench_current_size, zef.h_ES_workbench_relative_size);
catch
end
...
try
    zef_ui_bind_min_size(zef.h_ES_workbench, 500, 700);
catch
end
%%
