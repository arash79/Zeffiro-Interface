...
| Load / Save as / Save processed / Reset | pipeline `.mat` (v7.3 `zef_data` of `filter_*` fields + `raw_data`; **not JSON**), processed export, reset widgets. **Save as** writes the current `filter_*` lists including `filter_file_list`. **Load** reads the file chosen in the dialog. |
...
- Pipeline save/load is `.mat` v7.3, not JSON. Load and epoch-point load use the file chosen in the dialog.
