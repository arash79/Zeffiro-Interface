...
%     always append "; zef_ui_ready_new_windows; zef_update;".
%
...
for zef_i = 1 : size(zef.plugin_cell,1)
    ...
    cb = [zef.plugin_cell{zef_i,3} '; zef_ui_ready_new_windows; zef_update;'];
    if not(isempty(zef.h_menu_2))
        set(zef.h_menu_2,'Callback',cb);
    else
        zef.h_menu_2 = uimenu(zef.h_menu_1,'label',zef.plugin_cell{zef_i,1},'callback',cb);
    end
end
clear zef_i;
