               ...
               zef.([compartment_tag '_labeling_priority']) =  str2num(labeling_priority);
                ...
                if not(isempty(filename))
                    if not(isempty(foldername))
                        ...
                        % Check if filename already contains a path separator
                        if contains(filename, filesep) || contains(filename, '/') || contains(filename, '\')
                            % Filename already contains path, use it as-is relative to current directory
                            filePath = filename;
                        else
                            % Filename is just a name, prepend folder_name
