...
% Preserve current size state before opening (used by scaling logic below).
eval('zef.zeffiro_current_size_aux = zef.zeffiro_current_size;');
eval('zef.zeffiro_current_size = cell(0);');

% Resolve file and folder: either from arguments, user dialog, or zef fields.
...
if not(isequal(file_name,0));
    ...
    % Load the .fig file (MATLAB figure format) and strip tool-specific callbacks.
