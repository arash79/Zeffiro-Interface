...
%       'ShowChecks', 'Trigger' ('callback' | 'buttondown'),
...
switch action
    ...
    case 'create'
        ...
        nv.showchecks = local_tf(nv.showchecks);
...
try
    th = zef_ui_theme();
    panel.BackgroundColor = th.color.panel;
catch
end
try
...
show_checks = isstruct(ud) && isfield(ud, 'ShowChecks') && ud.ShowChecks;
if show_checks
    show_swatches = false;
end
if show_swatches
...
st.showchecks = 0;
if isfield(ud, 'ShowChecks')
    st.showchecks = double(ud.ShowChecks);
end
try
    th = zef_ui_theme();
    st.bg = th.color.panel;
    st.text = th.color.text;
    st.hover = th.color.hover;
    st.sel = th.color.rowSel;
    st.chip = th.color.panel;
    st.scroll = th.color.checkBorder;
    st.check = th.color.check;
    st.checkborder = th.color.checkBorder;
catch
end
ud.Suppress = true;
h.UserData = ud;
...
function local_apply_list_theme(h)
...
if local_is_uihtml(h)
    ...
    if ~isstruct(ud) || ~isfield(ud, 'Names')
        return
    end
    try
        th = zef_ui_theme();
        p = h.Parent;
        if isgraphics(p) && isprop(p, 'BackgroundColor')
            p.BackgroundColor = th.color.panel;
        end
    catch
    end
    local_push_uihtml(h);
...
try
    th = zef_ui_theme();
    h.BackgroundColor = [th.color.tableRow; th.color.tableAlt];
    h.ForegroundColor = th.color.text;
    h.FontColor = th.color.text;
catch
end
...
'html,body{margin:0;padding:0;height:100%;width:100%;background:var(--zbg,#fff);}' ...
'#list,#list .row{font-family:"Helvetica Neue",Helvetica,Arial,sans-serif;' ...
'font-size:12px;letter-spacing:-0.1px;color:var(--zfg,#243040);}' ...
'#list{height:100%;width:100%;overflow-x:hidden;overflow-y:auto;background:var(--zbg,#fff);' ...
'user-select:none;padding:0 2px 2px 0;box-sizing:border-box;display:flex;flex-direction:column;' ...
'align-items:stretch;scrollbar-width:thin;scrollbar-color:var(--zscroll,#c5cad3) transparent;}' ...
'#list::-webkit-scrollbar{width:5px;}' ...
'#list::-webkit-scrollbar-thumb{background:var(--zscroll,#c5cad3);border-radius:8px;}' ...
'#list::-webkit-scrollbar-track{background:transparent;}' ...
'.row{margin:0 8px 1px 0;padding:0 8px;border:0;height:18px;line-height:18px;' ...
'box-sizing:border-box;border-radius:8px;display:flex;align-items:center;flex:0 0 18px;' ...
'overflow:hidden;cursor:default;}' ...
'.row:hover{background:var(--zhover,#f3f5fb);}' ...
'.row.sel{background:var(--zsel,#e8eefc);}' ...
'.chip{display:inline-block;width:11px;height:11px;margin-right:8px;flex:0 0 11px;' ...
'border-radius:3px;border:0;}' ...
'.check{display:inline-flex;align-items:center;justify-content:center;' ...
'width:13px;height:13px;margin-right:8px;flex:0 0 13px;box-sizing:border-box;' ...
'border-radius:3px;border:1.5px solid var(--zcb,#c7d0d8);background:#fff;color:#fff;}' ...
'.check.on{background:var(--zck,#4b6ff5);border-color:var(--zck,#4b6ff5);}' ...
'.check svg{display:block;}' ...
'.lab{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;flex:1;}' ...
...
'var chips=d.showswatches!=0;var checks=d.showchecks!=0;' ...
'var box=document.getElementById("list");if(!box)return;' ...
'var root=document.documentElement;if(d.bg){root.style.setProperty("--zbg",rgbCss(d.bg));document.body.style.background=rgbCss(d.bg);box.style.background=rgbCss(d.bg);}' ...
'if(d.text){root.style.setProperty("--zfg",rgbCss(d.text));box.style.color=rgbCss(d.text);}' ...
'if(d.hover)root.style.setProperty("--zhover",rgbCss(d.hover));' ...
'if(d.sel)root.style.setProperty("--zsel",rgbCss(d.sel));' ...
'if(d.chip)root.style.setProperty("--zchip",rgbCss(d.chip));' ...
'if(d.scroll)root.style.setProperty("--zscroll",rgbCss(d.scroll));' ...
'if(d.check)root.style.setProperty("--zck",rgbCss(d.check));' ...
'if(d.checkborder)root.style.setProperty("--zcb",rgbCss(d.checkborder));' ...
'var html="";for(var i=0;i<names.length;i++){var on=sel.indexOf(i+1)>=0;' ...
'html+="<div class=''row"+(on?" sel":"")+"'' data-i=''"+(i+1)+"''>";' ...
'if(checks){html+="<span class=''check"+(on?" on":"")+"''>"+(on?' ...
'"<svg width=''10'' height=''10'' viewBox=''0 0 12 12''><path d=''M2.2 6.3 4.9 8.9 9.8 3.1'' fill=''none'' stroke=''#fff'' stroke-width=''1.9'' stroke-linecap=''round'' stroke-linejoin=''round''/></svg>":"")+"</span>";}' ...
'else if(chips){var c=colors[i]||[0.85,0.85,0.85];html+="<span class=''chip'' style=''background:"+rgbCss(c)+";''></span>";}' ...
'if(markers[i]){html+="<span class=''mark'' style=''color:"+rgbCss(mcols[i]||[0,0,0])+";''>"+esc(markers[i])+"</span>";}' ...
'html+="<span class=''lab''>"+esc(names[i])+"</span></div>";}box.innerHTML=html;' ...
'var hit=box.querySelector(".row.sel");if(hit){var t=hit.offsetTop,b=t+hit.offsetHeight;' ...
'if(t<box.scrollTop)box.scrollTop=t;else if(b>box.scrollTop+box.clientHeight)box.scrollTop=b-box.clientHeight;}' ...
