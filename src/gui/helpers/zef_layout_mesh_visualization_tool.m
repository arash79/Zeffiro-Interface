...
zef_ui_bind_min_size(fig, 640, 580);
...
for i = 1:size(pairs, 1)
    h = findobj(fig, 'Type', 'uilabel', '-and', 'Tag', ''); %#ok<NASGU>
    labs = findall(fig, 'Type', 'uilabel');
    for k = 1:numel(labs)
        try
            if strcmp(labs(k).Tag, pairs{i, 1}) || strcmp(char(labs(k).Text), local_old(pairs{i, 1}))
...
function local_move(fig, grid, names)

for i = 1:numel(names)
    h = local_by_prop(fig, names{i});
    if ~isempty(h) && isvalid(h)
        h.Parent = grid;
    end
end
