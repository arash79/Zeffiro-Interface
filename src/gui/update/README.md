...
**Granular widget → `zef` field sync** (and a few **zef → widget** refreshers). Complements the bulk table sync in `src/core/zef_update.m`. Triangle with the rest of the GUI stack: **init → open → update**.
...
- Look: `zef_update_contrast_and_brightness` (file; inner name may be `zef_update_contrast`), `zef_update_contrast`, `zef_update_brightness`, `zef_update_zoom`, `zef_update_ambience`, `zef_update_specular`, `zef_update_diffusion`, `zef_update_lights`, `zef_update_colormap`
...
`zef_update_forward_and_inverse_options`, `zef_update_graphics_options`, `zef_update_gaussian_prior_options`, `zef_update_options` — ValueChangedFcn from `zef_open_*`

### Segmentation / tables

`zef_update_compartment_table_data` (called from `zef_update`), `zef_update_transform`, `zef_update_transform_parameters`, `zef_update_parameters`, `zef_update_sensors_name_table`

### Parcellation / butterfly / EIT / labeling

`zef_update_parcellation` (**zef → widgets** refresh), `zef_update_butterfly_plot`, `zef_update_find_synthetic_eit_data`, `zef_update_labeling_priority`

### Pipeline (non-dialog)

`zef_update_lead_field_id`, `zef_update_source_positions`, `zef_update_parameter_distributions`
...
Figure creation also applies theme/layout via `src/gui/helpers` (`zef_ui_theme`, `zef_figure_tool_layout`); those are not replacements for these updaters.
...
- Filename quirks: `zef_update_contrast_and_brightness.m` vs inner function name.
