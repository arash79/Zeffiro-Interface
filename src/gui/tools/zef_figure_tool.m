...
% Ensure zef is in base workspace so callbacks can access it
% This is necessary because callbacks are evaluated in base workspace,
% but zef_figure_tool can be called from a function workspace (e.g., zef_start)
try
    assignin('base','zef',zef);
catch
    % If assignin fails (unlikely), callbacks will need to handle it via try-catch
end
...
% Ensure zef is in base workspace before drawnow (which may trigger callbacks)
% This ensures any callbacks that fire during drawnow can access zef.h_zeffiro
try
    assignin('base','zef',zef);
catch
    % If assignin fails, the callback's try-catch will handle it gracefully
end
