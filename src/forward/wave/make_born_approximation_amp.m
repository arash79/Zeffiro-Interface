...
for  j = 1 :  size(path_data,1)
    for k = 1 : size(path_data,2)-1
        ...
        if not(isequal(data_mode,'simulated'))
            ...
            diff_data_cos = rec_data_1{j,k+1}(1:ind_data_1) - rec_data_2{j,k+1};
            diff_data_cos = diff_data_cos(ind_data_0:data_resample_val:ind_data_1);
            diff_data_sin = rec_data_quad_1{j,k+1}(1:ind_data_1) - rec_data_quad_2{j,k+1};
            diff_data_sin = diff_data_sin(ind_data_0:data_resample_val:ind_data_1);
        else
            diff_data_cos = rec_data_1{j,k+1}(1:ind_data_1) - rec_data_2{j,k+1}(1:ind_data_1);
            diff_data_cos = diff_data_cos(ind_data_0:data_resample_val:ind_data_1);
            diff_data_sin = rec_data_quad_1{j,k+1}(1:ind_data_1) - rec_data_quad_2{j,k+1}(1:ind_data_1);
            diff_data_sin = diff_data_sin(ind_data_0:data_resample_val:ind_data_1);
        end
        diff_data_amp = abs(complex(diff_data_cos,diff_data_sin));
        diff_data_cos = repmat(diff_data_cos, [1 1 n_ast]);
        diff_data_sin = repmat(diff_data_sin, [1 1 n_ast]);
        diff_data_amp = repmat(diff_data_amp, [1 1 n_ast]);

        J_mat(size(path_data,1)*(k-1) + j, :, :) = (diff_data_cos.*aux_mat_cos(1, :, :) + diff_data_sin.*aux_mat_sin(1, :, :))./(diff_data_amp + eps_val_amp);
    end
end
...
save([torre_dir '/system_data/born_approximation_amp.mat'], 'J_mat', 'jacobian_ind', 'path_data', 't_data_jacobian', '-v7.3');
