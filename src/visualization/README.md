...
| `graph_bank/zef_logarithmic_histogram.m` | Log-y histogram of `log10(parameter_vec)` (filename ≠ first function name; MATLAB uses filename) |
...
Documented filename vs body mismatches (do not “fix” lightly): e.g. `zef_mean_energy_no_scaling` does mean-scale then `std`; `zef_std_no_scaling.m` does mean-scale then `corr`; `zef_cov_no_scaling` is DTW; some `corr_*` / `dtw_*` scale differently than the name suggests. Full table in prior documentation of this folder.
