...
| File | Kind | Role |
|------|------|------|
| `zef_create_finite_element_mesh.m` | function | Mesh-tool **Create FEM mesh** (`h_pushbutton21`). Chain: optional `zef_downsample_surfaces` → `zef_process_meshes` → `zef_create_fem_mesh` → `zef_postprocess_fem_mesh` → clear `source_ind` → rebuild the sensors table from `zef` → `zef_update` (copies mesh fields onto widgets). Does **not** assemble `zef.L`. |
