...
%   data using the Expectation-Maximization (EM) algorithm. It extends the
%   standard GMM formulation to support observation weights, making it suitable
%   for brain source reconstruction where source intensities serve as weights.
%
%   S = ADVGMMODELING4REC(X,WEIGHT,K,...) fits a K-component GMM to the data.
%   The fit maximizes the weighted log-likelihood of the data using
%   Expectation Maximization (EM). Rows of X correspond to observations,
%   columns correspond to variables. WEIGHT is an N-by-1 vector of non-negative
%   weights for each observation. The function returns estimated parameters in
%   a structure S containing the following fields:
...
%
%   [ ... ] = ADVGMMODELING4REC(X,weight,k,start,reps,CovType,SharedCov,RegV,
...
%
%   See also FITGMDIST, GMDISTRIBUTION, KMEANS.

%   Copyright 2007-2016 The MathWorks, Inc.
%   Adapted for Zeffiro Interface weighted GMM reconstruction.
...
%--------------------------------------------------------------------------
% GMCLUSTER_LEARN - Run a single EM iteration for Gaussian mixture fitting
% At this point the initial parameters must be provided.
%--------------------------------------------------------------------------
