%% ClassGMModeling - Gaussian mixture model-based source clustering for inverse solutions
%
% [MethodClassObj] = ClassGMModeling(MethodClassObj,reconstruction,zef,args)
%
% Fits a Gaussian mixture model (GMM) to the reconstruction to identify
% distinct source clusters. Supports both spatial location and orientation
% estimation, with multiple initialization strategies and model selection
% criteria (BIC, L2 density error, or fixed component count).
%
% Inputs:
%   MethodClassObj - inverse.CommonInverseParameters instance
%   reconstruction - Reconstruction data (vector or cell of time frames)
%   zef            - Zeffiro Interface project structure
%   args           - Name-value arguments (see arguments block)
%
% Outputs:
%   MethodClassObj - Updated with GMM results in MethodClassObj.GMM
%
...
% Parse covariance and cluster parameters for GMM fitting
...
% Optionally restrict estimation space to selected parcellation regions
...
for t=t_start:T
    ...
    % Compute current density magnitudes and unit direction vectors
    if iscell(z_vec)
    ...
    J = z;  % Current density magnitude (preserved for amplitude estimation)
    z = J/max(J);
    ind = z<threshold;
    z(ind) = 0;

    % Optional Gaussian smoothing via k-NN neighborhood averaging
    if smooth_std > 0
    ...
    if strcmp(initial_mode,'1')
        % k-means++ initialization: fit GMM with plus seeding
        try
    ...
    elseif strcmp(initial_mode,'2') || strcmp(initial_mode,'3')
        ...
        % Use partition-based initialization from cluster indices
        try
            GMModel_aux = FitAdvGMM(activity_space,weight,k,'CovarianceType',Sigma, ...
