...
%ZEF_KF Main Kalman filter reconstruction pipeline for Zeffiro Interface.
%
%   ZEF = ZEF_KF(ZEF) runs the Kalman filter reconstruction using parameters
%   from the ZEF structure. Supports multiple filter types: standard Kalman,
%   EnKF (ensemble), and sLORETA-weighted variants.
%
%   ZEF = ZEF_KF(ZEF, Q_VALUE) optionally supplies a fixed or time-varying
%   process noise covariance Q (scalar, vector, or matrix) instead of
%   computing it via find_evolution_prior.
%
%   Inputs:
%     ZEF      - Zeffiro project structure with reconstruction parameters
%     Q_VALUE  - (Optional) Process noise; overrides evolution prior computation
%
%   Output:
%     ZEF - Updated with zef.reconstruction and zef.reconstruction_information
%
%   Filter types (zef.filter_type):
%     1 - Standard Kalman filter
%     2 - Ensemble Kalman filter (EnKF)
%     3 - Kalman filter with sLORETA resolution weighting
%     4 - Kalman filter with alternative sLORETA weighting (requires theta0)
%
%   See also KALMAN_FILTER, KALMAN_FILTER_SLORETA, EnKF, RTS_SMOOTHER.
