...
| File | Role |
|------|------|
| `zef_sl1_map_estimation.m` | INI entry; function name `sl1_map_estimation` |
...
2. Launch via menu or `sl1_map_estimation(zef)` / `zef_sl1_map_estimation`.
3. Set `sl1_hyperprior`, type, SNR, iterations, time/filter; Start.
4. Inspect `zef.reconstruction` and `reconstruction_information`.

## Important notes

- Filename is `zef_sl1_map_estimation.m` but declared function is `sl1_map_estimation` (INI must match callable name on path).
- Window dump may still name an old `sl1_iteration`; `zef_init_sl1` overrides Start.
