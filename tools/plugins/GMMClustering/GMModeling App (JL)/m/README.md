...
| `BasicGMM/zef_GMModeling.m` | Related basic modelling entry |
...
| `AdvancedGMM/` | `zef_AdvGMModeling`, `FitAdvGMM`, `AdvGMModeling4Rec`, E-step helpers |
| `AdvancedGMM/estep.m` / `EstepWeight.m` / `WeightedCondDensity.m` | Vendor-style EM utilities (not public Zeffiro APIs) |
...
**Inputs.** Needs Statistics Toolbox (`fitgmdist` and related) and a populated `zef.reconstruction` (post-inverse). Thresholding / component-count syntax for time series is documented in the legacy `../README` note (comma / semicolon / space lists).
...
|--------------|-------------|
| `tools/plugins/GMModel` (SP) | Simpler SP GMM tool; some studies use it instead of JL |
| `plugins.ClassGMM` | Separate class plugin; unused from this GUI |
...
- AdvancedGMM vendor helpers (`estep`, `FitAdvGMM`, etc.) are implementation details, not stable public APIs.
...
- Avoid adding a third parallel GMM implementation under this plugin.
- Preserve INI callback name **`GMModelApp_start`**.
- Prefer writing results only under `zef.GMM.*` so SP overlays stay distinct.
- When adding parsers for component-count or color edit fields, update the legacy `../README` syntax notes.
- Do not `addpath` AdvancedGMM vendor files as general utilities outside this app.
