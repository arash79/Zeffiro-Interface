...
Preserve callback `zef_GMModel_start` and outputs on `zef.GMModel.*`. Keep Run callback wiring on `zef.GMModel.h_start`. `plugins.ClassGMM` remains unused from this GUI.
