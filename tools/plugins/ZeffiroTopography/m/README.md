...
Forward tools → Topography tool. Needs surfaces and measurements (or related sensor data) consistent with `top_*` time/band settings. `fig/` is unused leftover layout, not the live window.
...
- Leftover `fig/` assets are not the live UI.
