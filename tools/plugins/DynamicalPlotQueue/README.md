...
- Start file: `zeffiro_interface_dynamical_plot_queue` (function inside is `zef_dpq_start`) → `zef_dpq_window`
...
Preserve start entry `zeffiro_interface_dynamical_plot_queue` / `zef_dpq_start`. New bank helpers under `m/dynamical_plot_queue_bank/` should expose useful `help()` text for List descriptions. Keep `zef_plot_dpq` contract with visualization callers.
