...
%EXP_EM_ITERATION_MULTIRES  EXP EM MAP with RAMUS coarsening (GUIDE).
%
%   Zeffiro Interface.
...
%
%   [z, reconstruction_information] = exp_em_iteration_multires(void)
%
%   Called from EXP EM RAMUS GUIDE Start (not on the default menu).
%   Argument void is unused. Reads base zef.exp_multires_* lattices
%   (from exp_make_multires_dec) plus exp_em_multires_q / hyper_type /
%   beta / theta0. L via zef_processLeadfields(source_direction_mode).
%   SNR: zef.inv_snr → 10^(-inv_snr/20). Tag 'EXP EM Multiresolution'.
%   No inverse.*Inverter.
%
%   See also exp_em_map_estimation_multires, exp_make_multires_dec.

h = zef_waitbar(0,1,['EM MAP iteration for EP.']);
