...
Solvers: `tools/plugins/Kalman/m/` (`zef_kf_start` → `zef_kf_open_window`, `zef_KF`, `kalman_filter`, EnKF / block helpers). Extra batch helpers under `Scripts/` and `clusterScripts/` are not this UI.
...
- `Scripts/` / `clusterScripts/` are not the menu Start path.
