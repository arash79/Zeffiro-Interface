...
%READ_GITMODULES Parse a .gitmodules file into an array of submodule structs.
%
% SUBMODULE_STRUCTS = read_gitmodules(GITMODULES_FILE, kwargs) parses the
% given .gitmodules file and returns one struct per [submodule "path"] block.
% Each struct contains the key-value pairs from that block; "path" entries
% also get "abspath" (relative to the file's directory) and "name" (last
% path component). Valid keys include "path", "url", "branch", "startupscript".
%
% Inputs:
%   gitmodules_file - (1,1) string, mustBeFile
%                      Path to the .gitmodules file.
%   kwargs.required_fields - (:,1) string, mustBeMember(["path","abspath","url","branch","startupscript"]), default ["path","url","branch","startupscript"]
%                      Fields to require (used for validation; all parsed keys are still returned).
%
% Outputs:
%   submodule_structs - (:,1) struct
%                       Array of structs, one per submodule, with fields such
%                       as path, abspath, name, url, branch, startupscript.
%
