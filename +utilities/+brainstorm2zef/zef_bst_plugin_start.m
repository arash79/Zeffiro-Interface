...
% Configure Zeffiro system settings
zef.program_path = folder_name;
addpath(fullfile(folder_name,'m'));
zef = zef_apply_system_settings(zef);

% Calculate GUI window position (centered above segmentation tool)
position_vec = zef.segmentation_tool_default_position;
position_vec([2])  = position_vec([2])+position_vec([4]);  % Move up
screen_size = groot().ScreenSize;
position_vec = position_vec./screen_size([3 4 3 4]);  % Normalize to screen size
position_vec([3 4]) = 0.25;  % Set size to 25% of screen
position_vec([2]) = position_vec([2]) - 0.25;  % Adjust vertical position
...
% Discover available run scripts from main folder (they're now directly in +brainstorm2zef)
% Filter for functions that start with zef_bst_ and end with _fem_mesh_create
plugin_folder = fullfile(folder_name, '+utilities', '+brainstorm2zef');
all_files = dir(fullfile(plugin_folder, 'zef_bst_*_fem_mesh_create.m'));
if ~isempty(all_files)
    run_script_list = {all_files.name}';
% Extract base names (without .m extension)
...
% If no run scripts found, use default
if isempty(run_script_list)
    run_script_list = {'zef_bst_default_fem_mesh_create'};
end

% Set default run script selection
...
h_button_1 = uicontrol('Style','pushbutton','Parent',h_fig,'Visible','on','Units','normalized','Position',[0.70 0.80 0.25 0.15],'String','Settings file','Callback','utilities.brainstorm2zef.zef_bst_settings_file;');
h_button_3 = uicontrol('Style','pushbutton','Parent',h_fig,'Visible','on','Units','normalized','Position',[0.375 0.05 0.25 0.15],'String','Create project','Callback','utilities.brainstorm2zef.zef_bst_create_project(utilities.brainstorm2zef.zef_bst_get_settings_file_name,utilities.brainstorm2zef.zef_bst_get_project_file_name,utilities.brainstorm2zef.zef_bst_get_run_type,utilities.brainstorm2zef.zef_bst_get_input_mode,get(get(gcbo,''Parent''),''user_settings''));');
h_button_4 = uicontrol('Style','pushbutton','Parent',h_fig,'Visible','on','Units','normalized','Position',[0.05 0.20 0.25 0.15],'String','Edit project','Callback','utilities.brainstorm2zef.zef_bst_edit_project(utilities.brainstorm2zef.zef_bst_get_project_file_name);');
h_button_5 = uicontrol('Style','pushbutton','Parent',h_fig,'Visible','on','Units','normalized','Position',[0.05 0.05 0.25 0.15],'String','Edit settings','Callback','edit(utilities.brainstorm2zef.zef_bst_get_settings_file_name);');
...
callback_1 = sprintf('%s(%d,%d,utilities.brainstorm2zef.zef_bst_get_settings_file_name(h_fig),utilities.brainstorm2zef.zef_bst_get_project_file_name(h_fig),zef_bst);',['utilities.brainstorm2zef.' h_popmenu_1.String{h_popmenu_1.Value}],h_popmenu_2.Value,h_popmenu_3.Value);
callback_2 = sprintf('%s(%d,%d,utilities.brainstorm2zef.zef_bst_get_settings_file_name(h_fig),utilities.brainstorm2zef.zef_bst_get_project_file_name(h_fig),zef_bst);',['utilities.brainstorm2zef.' h_popmenu_1.String{h_popmenu_1.Value}],0,0);
