...
% The only inverter that defines a smoother today is KalmanInverter, whose
% smoother.m requires (self, z_inverse, L). Guard on use_smoothing so we
% don't enter the smoother branch (and its buggy interior) when smoothing
% wasn't requested, and pass bundle.L so the signature matches when it is.
