...
%
% Load a Gmsh mesh file (binary or ASCII) into a MATLAB
% struct called m. This file was copied from
%
%   https://github.com/simnibs/simnibs/blob/master/simnibs/matlab_tools/mesh_load_gmsh4.m
%
% at 2025-11-17, renamed and updated to support more modern
...
% Initialize data structures for optional mesh data fields
m.node_data = {};
m.element_data = {};
...
% Validate Gmsh file format: must start with $MeshFormat tag
tline = fgetl(fid);
if ~strcmp('$MeshFormat', tline)
    error('Start tag $MeshFormat expected');
end

% Parse mesh format header line: version.minor_version is_binary byte_size
% Format: "version.minor is_binary data_size"
tline = fgetl(fid);
version = sscanf(tline, '%f %d %d');
v = int32(version(1));  % Major version number

% Validate mesh version (only Gmsh 2.x and 4.x are supported)
if v ~= 2 && v ~= 4
    error(['Cannot read mesh version ' num2str(version(1))]);
end

% Validate data precision (must be 8 bytes for double precision)
if version(3) ~= 8
    error(['Expected 8 byte precision, but encountered: ' num2str(version(3))]);
end

% Determine if file is in binary or ASCII format
is_binary = version(2);
if is_binary
    disp('Reading binary format mesh')
    % Read endianness marker (1 = little-endian, required)
    endianess = fread(fid, 1, 'int');
    if (endianess ~= 1)
        error('Endianness in binary file is different than 1. Cannot proceed.\n Was this file created on Windows?');
    end
    % Read and validate line feed character (ASCII 10)
    a = fread(fid, 1, 'char');
    if a ~= 10
        error(['Expected LF (new line), but read ASCII: ' num2str(a)]);
    end
end

% Validate end of mesh format section
tline = fgetl(fid);
...
%% Read nodes section
% Skip to $Nodes tag (may have other sections in between)
line = fgetl(fid);
while ~strcmp('$Nodes', line)
    line = fgetl(fid);
end

% Read nodes based on format (binary/ASCII) and version (2.x/4.x)
if is_binary
   if v == 2
...
% Skip to $Elements tag
line = fgetl(fid);
while ~strcmp('$Elements', line)
    line = fgetl(fid);
end

% Read elements (triangles and tetrahedra) based on format and version
% continous_elm_numbers indicates if element numbering is sequential (required for element data)
if is_binary
    if v == 2
...
%% Read optional data sections ($NodeData, $ElementData, $ElementNodeData)
% These sections contain scalar/vector/tensor fields associated with mesh elements
tline = fgetl(fid);
while ~feof(fid)
    if strcmp('$NodeData', tline)
        % Node-associated data (e.g., electric potential, temperature)
        m.node_data{end+1} = read_node_data(fid, size(m.nodes,1), is_binary);
    elseif strcmp('$ElementData', tline)
        % Element-associated data (e.g., current density, field strength)
    ...
    elseif strcmp('$ElementNodeData', tline)
        % Element node data (data at nodes within elements, e.g., shape function values)
        if is_binary
            m.element_node_data = {};
            [data, name, ~] = read_data_binary(fid, tline);
            m.element_node_data{end+1}.data = data;
            m.element_node_data{end}.name = name;
            warning('Reading of element_node_data is not fully tested. Use with caution.');
        else
...
% Transpose cell arrays to column vectors for consistency
...
function m = read_nodes_binary(m, fid)
    % Read nodes from binary Gmsh 2.x format
    % Format: number_of_nodes (ASCII), then for each node: [node_id(int) x(double) y(double) z(double)]
    
    % Read number of nodes (ASCII line in binary format)
    tline = fgetl(fid);
    ...
    % Read first node ID to validate numbering
    node_number(1) = fread(fid, 1 , 'int');

    % Read all node coordinates: [x y z] for each node
    % Format: skip 4 bytes (node ID) before each coordinate triple
    m.nodes = fread(fid, [3 number_of_nodes], '3*double',4)';

    % Read last node ID to validate continuous numbering
    fseek(fid, -(2*4+3*8), 'cof');  % Rewind: 2 ints + 3 doubles
    node_number(2) = fread(fid, 1 , 'int', 3*8);

    % Validate node numbering is continuous starting from 1
    if (node_number(1) < 1) || ...
       (node_number(2) > number_of_nodes) || ...
       (size(m.nodes,1) ~= number_of_nodes)
        error('Node numbers must be a continuous list of indexes starting at 1');
    end

    % Handle optional line feed after binary node data
    read_LF(fid);

    % Validate end of nodes section
    tline = fgetl(fid);
...
function m = read_nodes(m, fid)
    % Read nodes from ASCII Gmsh 2.x format
    % Format: number_of_nodes, then for each node: node_id x y z
    ...
    % Read all node data: node_id x y z
...
function [m, continous_elm_numbers] = read_elements_binary(m, fid)
    % Read elements from binary Gmsh 2.x format
    % Only triangles (type 2) and tetrahedra (type 4) are supported
    % Returns flag indicating if element numbering is continuous (required for element data)
    ...
    % Read elements grouped by type
    while elm_remaining > 0
        type = fread(fid, 1, '*int32');              % Element type (2=triangle, 4=tetrahedron)
        nr_elm_follow = fread(fid, 1, '*int32');    % Number of elements of this type
        nr_tags = fread(fid, 1, '*int32');           % Number of tags (must be 2)
        
        if (nr_tags ~= 2)
            error('Number of tags must be equal to 2');
        end

        elm_remaining = elm_remaining - nr_elm_follow;
        
        % Read all elements of this type: [element_id, tag1(region), tag2, node1, node2, ...]
        % Format: element_id (int), tag1=region (int), tag2 (int), node indices (int)
        buff = fread(fid, [nr_nodes_in_element(type)+3, nr_elm_follow], '*int32');
        ...
        switch type
            case 2  % Triangle elements
                if ~isempty(buff)
                    ...
                    m.triangle_regions = [m.triangle_regions; buff(2,:)'];  % tag1 is region label
                end

            case 4  % Tetrahedron elements
                if ~isempty(buff)
                    m.tetrahedra = [m.tetrahedra; buff(4:end,:)'];
                    m.tetrahedron_regions = [m.tetrahedron_regions; buff(2,:)'];  % tag1 is region label
                end

            otherwise
                % Unsupported element type - skip and disable element data reading
    ...
    % Validate element numbering is continuous (required for element data)
    if (min(element_numbers) < 1) || ...
       (max(element_numbers) > number_of_elements)
       continous_elm_numbers = false;
       warning('Element numbering is not continuous. Element data will not be read.');
    end

    % Validate element ordering: triangles must come before tetrahedra
    if any(element_types==2) && any(element_types==4) && ...
       find(element_types==2, 1, 'last') > find(element_types==4, 1)
       continous_elm_numbers = false;
       warning('Triangles and tetrahedra are not in expected order. Element data will not be read.');
    end

    % Handle optional line feed after binary element data
    read_LF(fid);

    % Validate end of elements section
    tline = fgetl(fid);
    if ~strcmp('$EndElements', tline)
        error(['End tag $EndElements expected, but read: ' tline]);
    end
...
function read_LF(fid)
    % Handle optional line feed (LF, ASCII 10) after binary data sections
    % Some Gmsh files include LF, others go directly to next section marker "$"
    a = fread(fid, 1, 'char');
    if a == 10
        % Line feed found - this is expected and handled
        % (No action needed, file pointer is now at next section)
    elseif a == 36
        % ASCII 36 = '$' - no LF present, rewind to read section marker
        fseek(fid, -1, 'cof');
    else
        error(['Unexpected character after binary data: ASCII ' num2str(a)]);
    end
end % function

function n = nr_nodes_in_element(t)
    % Return number of nodes for a given Gmsh element type
    % Element types: 1=line, 2=triangle, 4=tetrahedron, etc.
    % This lookup table maps Gmsh element type codes to node counts
    nne = ...
     [ 2,  3,  4,  4,  8,  6,  5,  3,  6,  9, ...
